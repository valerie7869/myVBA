...
End Sub
Option Explicit
Sub CreateReportMAC_v5_1()
' createReport4.1
' last dist: OpenLogicMacros2015-0819  (must change at bottom for summary page transfer)
...
'Optimize Code
   ...
   'Application.ScreenUpdating = False  ' turn off screen updates while running --- NOW COVERED IN OPTIMIZE CALL ABOVE
    'work on Licenses sheet
    Sheets("Licenses").Select  'go to License tab
    ' create table for files tab
    Set rng1 = Range(Range("A1"), Range("A1").SpecialCells(xlLastCell))
...
ThisWorkbook.Worksheets("Checklist (internal)").Copy _
    Before:=Application.ActiveWorkbook.Sheets("Pivot_SoftwareModel")
    
ThisWorkbook.Worksheets("MasterLicenseTable").Copy _
    Before:=Application.ActiveWorkbook.Sheets("Pivot_SoftwareModel")
  
ThisWorkbook.Worksheets("Summary").Copy _
    Before:=Application.ActiveWorkbook.Sheets("Pivot_SoftwareModel")

ThisWorkbook.Worksheets("BOM (Bill of Material)").Copy _
    Before:=Application.ActiveWorkbook.Sheets("Pivot_SoftwareModel")
    
ThisWorkbook.Worksheets("Non OSS").Copy _
    Before:=Application.ActiveWorkbook.Sheets("Pivot_SoftwareModel")

ThisWorkbook.Worksheets("by License").Copy _
    Before:=Application.ActiveWorkbook.Sheets("Pivot_SoftwareModel")
    
ThisWorkbook.Worksheets("Obligations Summary").Copy _
    Before:=Application.ActiveWorkbook.Sheets("Obligations")
    
  ' come back to report
ActiveWorkbook.Sheets("Checklist (internal)").Select   'land here on checklist page upon exit
Range("A1").Select
'
'/// prepare exit   ///////////////////////////////////////////////////////////////
    Application.StatusBar = ""   ' clear
    
    'Optimize Code exit-reset
    Call OptimizeCode_End
    
    ActiveWindow.TabRatio = 0.999  ' make wider the tab view along bottom
    'Application.ScreenUpdating = True  ' turn back on screen
